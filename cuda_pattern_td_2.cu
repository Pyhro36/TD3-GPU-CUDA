...
#define O_TILE_MAX_I ( I_TILE_HEIGHT - STENCIL_EDGE )
#define O_TILE_MAX_J ( I_TILE_WIDTH - STENCIL_EDGE )
...
__global__ void stencil(float *output, float *input, int width, int height, int depth) {
    ...
    int ii = tx + (blockDim.x * blockIdx.x); // -> height
    int yy = ty + (blockDim.y * blockIdx.y); // -> width * depth

    if ((ii < height) && ((yy / depth) < width) && ((yy % depth) < depth))
    {
        privateInput[(tx * I_TILE_WIDTH * I_TILE_DEPTH) + ty] = input[(ii * width * depth) + yy];
    }
    // comme on ne se soucie pas des bornes, on utilise pas les valeurs en dehors de height et width de privateInput

    // on prend les threads de STENCIL O_TILE_MAX__ pour calculer les output
    if ((tx > 0)
    && (tx < O_TILE_MAX_I)
    && (privateJ > 0)
    ...
    {
        res = privateInput[(tx * I_TILE_WIDTH * I_TILE_DEPTH) + ty + 1]      // [i,j,k+1]
                + privateInput[(tx * I_TILE_WIDTH * I_TILE_DEPTH) + ty - 1]     // [i,j,k-1]
                + privateInput[(((tx * I_TILE_WIDTH) + 1) * I_TILE_DEPTH) + ty] // [i,j+1,k]
                + privateInput[(((tx * I_TILE_WIDTH) - 1) * I_TILE_DEPTH) + ty] // [i,j-1,k]
                + privateInput[((tx + 1) * I_TILE_WIDTH * I_TILE_DEPTH) + ty]   // [i+1,j,k]
                + privateInput[((tx - 1) * I_TILE_WIDTH * I_TILE_DEPTH) + ty]   // [i-1,j,k]
                - (6 * privateInput[(tx * I_TILE_WIDTH * I_TILE_DEPTH) + ty]);  // [i,j,k]

        if (res < 0.0f)
        {
            output[(ii * height * width) + yy] = 0.0f;
        }
        else if (res > 255.0f)
        {
            output[(ii * height * width) + yy] = 255.0f;
        }
        else
        {
            output[(ii * height * width) + yy] = res;
        }
    }
    else
    {
...
static void launch_stencil(float *deviceOutputData, float *deviceInputData, int width, int height, int depth) {
    ...
    //@@ INSERT CODE HERE
    // on reunit width et depth pour avoir une grille 2D comme demande dans l'enonce
    dim3 gridDim(1 + ((height - 1) / I_TILE_HEIGHT),
            (1 + ((width - 1) / I_TILE_WIDTH)) * (1 + ((depth - 1) / I_TILE_DEPTH)), 1);
